...
- The result is a fixed file of numbers. Using it does not change it:
  your chats do not update the parameters.
...
sentence. The last bullet is the one students get wrong: the model
does not learn from your chat. Whatever it seems to remember is in the
conversation in front of it, or in a note the app saved and pasted
back in.
:::
...
Two stages of training produce the model. Both change only the
parameters.
...
**The result is a file.** A trained model is a fixed set of numbers,
copied onto servers. Using it does not change it. A chat does not
update the parameters, and nothing you tell it in one conversation is
there in the next, unless the app saved a note and pastes it back in.
The next section explains that "unless".
...
- **Model**: the file of numbers. Nobody types into it directly.
- **Harness**: the program around the model. It assembles what the
  model sees (a hidden **system prompt**, your messages, the attached
  file), sends it, and shows you the reply.
- **Tools**: a harness can let the model call programs: web search, a
  ...
  harness runs it and pastes the result back in.
- **Agent**: a harness that lets the model call tools in a loop until
  the task is done.
...
You never talk to a model. You talk to a **harness**: the program that
wraps the model and turns it into a product. The harness writes a
hidden **system prompt** (instructions on how to behave, today's date,
