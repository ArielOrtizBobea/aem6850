...
1. Four facts about how a model works
...
## Four facts about how a model works {#four-facts}

- It predicts the next word. Plausible is not the same as correct.
- It generates; it does not look anything up. That is where invented
  references come from.
- It is not deterministic. A transcript is not something anyone can
  re-run.
- Its memory of a conversation is finite. Long chats degrade without
  saying so.
...
- A reference, a function argument, a number: it produces text shaped
  like the thing.
- Often the text matches something real. Sometimes it matches nothing.
...
- A reference from a chatbot is a lead to check, not a citation.
...
- The next word is sampled from the list. Chat apps fix how much; you
  cannot turn it off.
- Same prompt, same tool, same minute: two students, two replies.
- Exercise 1 shows this in the room.
- A transcript is not a record anyone can re-run. The script you saved
  is.
...
- As it fills, the app compresses earlier messages into a summary, with
  no notice.
- At message thirty the tool has not got worse; the code you pasted at
  the top is no longer there verbatim.
- One task per chat. Paste the code again; do not write "the code from
  before".
...
|:--|:--|:--|:--|
| Predicts the next word | Plausible before correct | Check the claim against the thing | Read the explanation against the code |
| Generates, does not look up | Invented references and arguments | Verify before you use | Paste the DOI; run the line |
...
| Finite memory of a chat | Long chats degrade | One task per chat; paste again | A new chat for each task |
...
| Coding agent | Claude Code | a repository; edits as diffs | each diff before you accept it |
...
## What chat is for, and what it cannot do {#chat-uses}
...
- "Fix only the line I wrote" closes the scope: one line, not a new
  approach.
- Check: run the fixed line. `lifeExp`, capital E.
...
gapminder-practice/
  ...
  data/gapminder.csv          1704 rows, 6 columns
  code/                       one script per task (empty for now)
  notes.md                    added later, from the page
```

::: {.content-visible when-format="revealjs"}

- Open the `.Rproj`, so `data/gapminder.csv` resolves.
- The CSV is the only input. Nothing ever edits it.
- `output/` appears when a script writes there.

::: {.notes}
This folder is where everything today happens: one input file, a notes
file, and a code folder the exercises fill.
:::
...
Make the folder now; five minutes. In RStudio: **File → New Project →
New Directory → New Project**. Directory name `gapminder-practice`, as a
subdirectory of your `github` folder (never Desktop or Documents if those
sync to a cloud drive). **Create Project**. In the Files pane click
**New Folder** twice: `data` and `code`. Download [gapminder.csv](data/gapminder.csv) (right-click the
link: *Save link as*, or in Safari *Download Linked File*) into
`github/gapminder-practice/data/`; if the browser put it in Downloads,
drag it there. The test, in RStudio's Console with the project open:
`nrow(read.csv("data/gapminder.csv"))` prints `[1] 1704`. `cannot open
file` means the CSV is not in `data/`.

`gapminder-practice/` is an RStudio project in your `github` folder.
...
gapminder notes
- csv from the course site, 1704 rows? check
...
- Goal, failure mode, artifact, what not to do: every prompt today has
  the four.
...
press Enter. The prompt has the four parts. The goal: an explanation,
one line at a time, at your level. The failure mode to avoid: a rewrite
instead of an explanation. The artifact: a quoted line followed by a
sentence. What not to do: suggest changes. "For someone new to R" is in
...
#    page (code/1_lifeexp_2007.R). File > Save As, into the code
#    folder, named 1_lifeexp_2007.R. Click Source. Read the five
...
Files pane; [The practice folder](#practice-folder) has the download
link.
...
- `Error: nrow(d) == 1704 is not TRUE`: the CSV in `data/` is not the
  course file (a partial download). Download it again from the page.
...
| With the artifact declared: a script at `code/...`, what it reads, what it prints | a file you can run and keep |
...
base R named) it returns a line of code that fits your columns. With the
artifact declared (a script at `code/...`, what it reads, what it
prints, "do not compute it here") it returns a file you can run and
keep. Same question, three different objects come back. The three
...
The prompt with the artifact declared.
...
- **Goal**: the number or file you want, stated once.
- **Failure mode to avoid**: the mistake a plausible answer makes here.
- **Artifact and path**: a script at `code/...`, what it reads, writes,
  prints.
- **What not to do**: no numbers in the chat; no invented columns; stop
  if the file differs.

::: {.content-visible when-format="revealjs"}

::: {.notes}
A prompt has four parts. The second one is the one nobody writes, and it
is the one that saves you the wrong-years recipe.
:::
...
The anatomy every prompt on this page follows, each part with the
example from the Exercise 2 prompt. The goal: the population-weighted
mean of `lifeExp` by continent in 2007. The failure mode to avoid: twelve
years per country, so filter first; do not take the simple mean. The
artifact and its path: an R script at `code/2_lifeexp_weighted_2007.R`,
what it reads, what it writes, what it prints. What not to do: no numbers
in the chat, no invented columns, stop if the file does not match.
...
goal. The third names the failure mode twice (filter to 2007 first;
weight by `pop`, not the simple mean) and the artifact with its path,
what it reads, writes and prints. The last paragraph says what not to
...
:::: {.columns}
::: {.column width="60%"}
![](figures/08-ai-chat/S06-desktop-app-cowork.png){width="100%" fig-alt="The Claude desktop app on a Pro account, with the Cowork choice in the header or composer circled in red."}
:::
::: {.column width="40%"}
1. **Cowork**: the assistant tier, in the desktop app
2. **Reads** every file in a folder you connect
...
![The Claude desktop app with Cowork available. On the free plan this choice is absent.](figures/08-ai-chat/S06-desktop-app-cowork.png){fig-align="center" fig-alt="The Claude desktop app on a Pro account, with the Cowork choice in the header or composer circled in red."}
...
:::: {.columns}
::: {.column width="60%"}
![](figures/08-ai-chat/S07-cowork-folder-connected.png){width="100%" fig-alt="Cowork in the Claude desktop app with the gapminder-practice folder connected; the folder name, the task box, the stop control and the approval-mode setting are circled in red."}
:::
::: {.column width="40%"}
1. **The folder**, named in the window
2. **The task box**: where the prompt goes
3. **Stop**: find it first
4. **Approvals**: leave it asking
:::
::::

::: {.notes}
Find three things before you type anything. The third one is the stop
button.
:::
...
![Cowork with the practice folder connected. Four things to find before typing: the folder's name, the task box, the stop control, the approval setting.](figures/08-ai-chat/S07-cowork-folder-connected.png){fig-align="center" fig-alt="Cowork in the Claude desktop app with the gapminder-practice folder connected; the folder name, the task box, the stop control and the approval-mode setting are circled in red."}

The click path. Open the desktop app and choose Cowork. Choose the
control that connects a local folder and pick
`github/gapminder-practice`. Before typing, confirm three things. The folder's name
shows in the window. The task box is visible. You have found the stop
control before you need it. Permission mode: leave it at the setting
that asks before acting (Manual).
