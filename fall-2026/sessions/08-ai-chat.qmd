...
- Using LLMs for research erquires using them for writing code (reproducible), not generateing output directly (not reproducible) 
...
Make the folder now; five minutes. Everything today reads one file,
`gapminder.csv`.
...
dir.create("data")
dir.create("code")
url <- "https://arielortizbobea.github.io/aem6850/fall-2026/sessions/data/gapminder.csv"
download.file(url, "data/gapminder.csv")
...
folder the exercises fill. Wait for the 1704 around the room. Nobody
retypes the URL: copy the line from the slide or the page.
`download.file()` prints a few lines about the download first.
`cannot open file` means the CSV is not in `data/`: run the download
line again. Windows: the parent folder is typed as
...
folder instead. Step 2 makes the two folders and fetches the CSV from
the course site into `data/`. `download.file()` prints a few lines about
the download first; the `1704` is the test. If the download line fails
(no network, a proxy), download [gapminder.csv](data/gapminder.csv) with
the browser (right-click the link: *Save link as*, or in Safari
*Download Linked File*) into `github/gapminder-practice/data/`, then run
the last line again. `cannot open file` means the CSV is not in `data/`.
...
- `Error: nrow(d) == 1704 is not TRUE`: the CSV in `data/` is not the
  course file (a partial download). Run the `download.file()` line again.
