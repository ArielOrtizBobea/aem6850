...
How to hand one function to a coding tool that edits files in your
repository, and how to know whether what came back is right. State the
goal and the mistake to avoid; work out the answer by hand on a small
case; run it, read the diff, and leave the check in the script. Every
step runs in RStudio's Terminal tab inside a git repository.
...
needs a Claude Pro subscription; the free plan does not include it. Do
all six steps; step 6 is the test.

**1. Subscribe to Claude Pro** ($20 a month). You will get a lot out of it
this semester; it is worth the investment. Cornell is working on access
for students, but nothing is in place as of September 2026, so plan on
Pro for now: choose Pro on the [pricing page](https://claude.com/pricing).
If you decide not to subscribe, follow
the captured run on the slides in class and pair with a neighbour for
the two exercises.
...
`claude --version`, Enter. It prints a version number ending in
`(Claude Code)`, like `2.1.258 (Claude Code)`. `command not found`: quit
...
```r
dir.create("data")
download.file("https://arielortizbobea.github.io/aem6850/fall-2026/sessions/data/gapminder.csv", "data/gapminder.csv")
nrow(read.csv("data/gapminder.csv"))
#> [1] 1704
```

`download.file()` prints a few lines about the download first; the
`1704` is the test.
...
::: {.notes}
Four things, and you do each of them on your own laptop in the two
exercises.
:::
...
1. Coding tools that read your files
2. The five-step loop and two rules
3. Starting Claude Code in the repository
4. Delegating one function
5. Reading the diff, the patch, the check
6. Version control from inside the agent
7. CLAUDE.md: standing context and your style
8. "Runs clean" is not "is right"
...
## Inline and agentic coding tools {#coding-tools}
...
- Every screen here is Claude Code. The habits are the category's.

::: {.notes}
The agentic column can create and change files on your disk. That is
both the capability and the risk.
:::
...
Two kinds of coding tool exist today, and they behave differently. An
inline tool lives in the editor and proposes the next few lines as grey
text while you type; [GitHub Copilot inside
RStudio](https://docs.posit.co/ide/user/ide/guide/tools/copilot.html)
is one. An agentic tool lives in a terminal. It reads the folder you
start it in, writes files, runs commands, and asks before each change.
Claude Code is one; Codex CLI, Gemini CLI and
Cursor's agent are peers. This page shows Claude Code because every
screenshot and every prompt here is one real run of one tool. The
habits transfer to the category.
...
patch, run again. Then write the hand check into the script as a
`stopifnot()` line, so the next run checks itself. The value on paper
comes from the data, not from a guess; that is what makes step 4 a
comparison and not an impression.

:::

## Two rules for letting a model touch your files {#two-rules}
...
something. A Pro account may
otherwise start in auto mode, where a second model approves edits
instead of you. The status line above the prompt says which mode you are
in; it must read `⏸ manual mode on` for everything today. Shift+Tab
cycles the modes; if the keystroke does not reach the tool inside
RStudio's terminal, quit with `/exit` and relaunch with the line above.
An older install that rejects `manual` accepts `--permission-mode
default`, which is the same mode.
...
:::
::: {.column width="40%"}
1. **Version**: `2.1.258` or later
2. **Model · plan**: Sonnet 5 on Pro; wording varies
...
![The welcome screen, with the empty middle of the terminal cut out. The captured account is not a student account; on a Pro account the second line names Sonnet 5 and Claude Pro, in wording that varies.](figures/09-ai-code/S02-welcome.png){fig-align="center" fig-alt="The Claude Code welcome screen: Claude Code v2.1.258, the model and plan line, the folder ~/github/gapminder-practice, the prompt box, and the status line reading manual mode on."}

Four lines matter: the version; the model and plan (a Pro account names
Sonnet 5 and Claude Pro; the captured account differs); the folder; and
...
Every Claude Code screen on this page comes from real runs of version
2.1.258, captured on 2026-09-14; the RStudio screens are separate
captures. Replies vary from run to run: the wording of a
summary, sometimes the code. The steps do not.

:::

## A read-only first prompt {#first-prompt}
...
- It reads; it does not edit. No edit box appears.
- A box for a listing command such as `ls` may appear: read it, `1. Yes`.
- Compare with the Files pane. Same files, same folders.
...
pane: same files, same folders. Then
`git status` in a second Terminal tab (next section) prints clean,
which is the proof that a read-only prompt read only.
...
## What the transcript says it did {#transcript-says}
...
## Three tests of a delegation {#three-tests}
...
## stopifnot(): the check stays in the script {#institutionalize}
...
- Checked: Oceania 2007 by hand, 81.06; stopifnot() in the script
```
...
to ask the tool for the check?* The prompt is under the `stopifnot()`
section above. Read the `stopifnot()` line in the box against your paper
...
decimal check uses a tolerance, as in the `stopifnot()` section.
...
## The diff is on screen before the file changes {#diff-on-screen}
...
::: {.notes}
Every edit arrives as a diff in the tool itself. The desktop app's Code
tab shows the same diff in a window. Reading it here and reading it in
`git diff` are the same act.
:::
...
the lines; Esc closes. The Claude desktop app's
Code tab shows the same diffs in a window, with Accept and Reject
buttons. Whichever window you read it in, the diff is the same object
`git diff` prints in Terminal 2.
...
## The agent runs git when you ask {#agent-git}
...
git command is shown and approved. In Auto mode on a Pro plan, commits
and pushes in the same repository run without a prompt: one more reason
to stay in manual mode until the habit is formed.
:::
...
mode would have done. A Pro session may start in Auto mode. There,
commits and pushes to the repository you are working in run without a
prompt. That is one more reason to check the status line and stay in
manual mode until reading diffs is a habit.
...
## CLAUDE.md: read at every launch {#claude-md}
...
## The file it produced: conventions applied unasked {#short-prompt-file}
...
## What Claude Code is bad at {#bad-at}
...
- Long stretches without reading. Accept-edits mode is not for this
  month.
...
stretches without reading; accept-edits mode exists and is not for this
month.
...
## "Runs clean" is not "is right" {#runs-clean}

- The first script ran clean, printed five plausible numbers, and
  answered the wrong question.
- The number on paper caught it and the diff showed why; the
  `stopifnot()` line keeps it caught.
- The bottleneck is verification, not typing. Faster tools move it to
...
the diff made that visible, and only the `stopifnot()` line keeps it
...
**Links:** [slides](09-ai-code-slides.html) · [Claude Code docs](https://code.claude.com/docs/en/quickstart) · [Syllabus](../syllabus.qmd) · [data/gapminder.csv](data/gapminder.csv)
