...
- Put the check in the prompt and let the tool run until it passes, or
  hand a review to a read-only subagent, and say when each fits.
...
8. Loops and subagents: the tool checks itself
9. A clean run that is wrong
...
results. For most of this page the third phase is yours: the
delegations tell the tool not to run the script, in the prompt or in
`CLAUDE.md`, and you run it in RStudio. The section on loops hands that
phase to the tool, with a check it can run.
...
## A prompt without a loop and a prompt with one {#loop-prompts}

| | Without a loop: every prompt so far | With a loop |
|:--|:--|:--|
| The check | on your paper | in the script, from your paper |
| Who runs the script | you, in RStudio | the tool, with `Rscript`, until the check passes |
| Who reads a failure | you; then a patch request | the tool; it patches and runs again |
| What you read afterwards | the diff | the diff, and the output of the last run |

```{.markdown filename="Prompt to paste into Claude Code (captured run, with a loop)"}
Write code/gdp_by_continent.R. It defines gdp_by_continent(d, yr): the mean
gdpPercap of each continent in year yr, as a named numeric vector with one
number per continent. Base R only. At the bottom, read data/gapminder.csv into
d, store gdp_by_continent(d, 2007) in res, print res, and check it with
stopifnot(abs(res["Oceania"] - 32883) < 50). By hand, Oceania 2007 is 32,883:
Australia 34,435.37 with 20,434,176 people and New Zealand 25,185.01 with
4,115,771 people. Then run the script with Rscript code/gdp_by_continent.R. If
the check fails, change the function so that the check passes and run again.
Show me the output of the last run. Do not use git.
```

::: {.content-visible when-format="revealjs"}

::: {.notes}
Same function as practice task 4, and the prompt is ambiguous on
purpose: "the mean gdpPercap". What differs is where the check lives and
who runs the script.
:::
...
Every prompt so far ended with "Do not run the script; I will run it
myself", and you were the loop: run, compare with the paper, ask for a
patch. The tool can be the loop instead, if the prompt gives it a check
it can run and tells it to run until the check passes. Anthropic's
[best practices](https://code.claude.com/docs/en/best-practices) call
this giving Claude a way to verify its work: a check that returns pass
or fail, so that the loop closes on its own. Here the check is the hand
check you have been typing since the first delegation, written into the
script and named in the prompt.

The prompt above is the captured run's. It is ambiguous on purpose,
"the mean gdpPercap of each continent", the same gap as the first
delegation of the day. The check carries the hand value, 32,883, from
the two Oceania rows (practice task 4 works it out). The prompt also
says how to run the script and what to show at the end: the output of
the last run is the evidence, and the docs say to ask for evidence
rather than a claim of success.

:::

## The loop's run: a failed check, a patch, a pass {#loop-run}

```{.default filename="The captured run, condensed; the full transcript is on the page"}
Write(code/gdp_by_continent.R)        tapply(sub$gdpPercap, sub$continent, mean)
Bash  Rscript code/gdp_by_continent.R
        Africa  Americas      Asia    Europe   Oceania
      3089.033 11003.032 12473.027 25054.482 29810.188
      Error: abs(res["Oceania"] - 32883) < 50 is not TRUE
Edit(code/gdp_by_continent.R)  tapply(sub$gdpPercap * sub$pop, sub$continent, sum) /
                                 tapply(sub$pop, sub$continent, sum)
Bash  Rscript code/gdp_by_continent.R
      2560.930 21602.746  5432.372 25244.048 32884.555
```

- Two runs, one patch, no prompt from you in between.
- Its reply: the check only passes for the population-weighted mean, so
  that is what the final function computes.
- The check is now the specification. A wrong check gives wrong code
  that passes.

::: {.content-visible when-format="revealjs"}

::: {.notes}
The morning's failure, caught and fixed without you. Ask the room what
29,810 in the check would have produced. In manual mode each edit and
each Rscript still asks: four boxes, `1. Yes` each.
:::
...
The run: it listed the folder, read the existing script for its
pattern, wrote a plain mean of `gdpPercap`, ran it, and the check
stopped the script at Oceania 29,810. It edited the function to weight
by `pop`, ran again, got 32,884.56, and replied with the output and two
flags: that the request said "mean" but the check only passes for the
weighted mean, and that `CLAUDE.md` says not to run scripts but the
prompt asked. Two runs, one patch, and you typed nothing in between.

The lesson is in the first flag. With a loop, the check is the
specification: the tool changes the code until the check passes, and it
will make a wrong check pass too. With 29,810 in the check, the simple
mean, the first run passes, the function stays unweighted, and the reply
reports success. So the value in the check comes from your hand, from
rows you can see, exactly as in step 3 of the loop; the tool's loop
does not replace that step, it depends on it.

When to use a loop: when you have a check you trust and the tool can
run, a hand value, a row count, a column list, a file that must exist.
When not: when the check is the thing you are unsure of, or when a run
has side effects you have not named, files written, downloads, long
jobs. After a loop you still read the diff with the five questions;
the tool's own summary is a transcript, and the diff is the fact. The
docs describe stronger versions of the same loop, a goal the tool
re-checks after every turn and a rule that stops it from finishing
until a script passes; for one function, the prompt is enough.

In manual mode each edit and each `Rscript` arrives as a box: a *Create
file* box, a *Bash command* box, then the failed run in the transcript,
an *Edit file* box with the red and green lines, a *Bash command* box,
then the passing run. Four `1. Yes`. The run below was captured
non-interactively with those tools allowed in advance, which is the
same thing without the boxes.

::: {.callout-tip collapse="true"}
## The full captured transcript

````{.default}
⏺ Bash(ls code data && head -3 data/gapminder.csv)
  ⎿ code:
    lifeexp_by_continent.R
    lifeexp_gain.R
...
⏺ Read(code/lifeexp_by_continent.R)
  ⎿ 1	# Population-weighted mean life expectancy by continent for a given year (base R only)
    2	
    3	lifeexp_by_continent <- function(d, yr) {
    4	  sub <- d[d$year == yr, ]
    5	  tapply(sub$lifeExp * sub$pop, sub$continent, sum) /
    6	    tapply(sub$pop, sub$continent, sum)
    7	}
    8	
    9	d <- read.csv("data/gapminder.csv", stringsAsFactors = FALSE)
    10	res <- lifeexp_by_continent(d, 2007)
    11	print(res)
    12	
    13	# Check: Oceania 2007 by hand. Australia 81.235 (20,434,176 people) and
    14	# New Zealand 80.204 (4,115,771 people) give 81.06.
    15	stopifnot(abs(res["Oceania"] - 81.06) < 0.01)
    16
⏺ Reply:
  Writing the script as specified — a plain mean of `gdpPercap` per continent — then running it.
⏺ Write(code/gdp_by_continent.R)
  # Mean GDP per capita by continent for a given year (base R only).
  # Prints a named vector with one number per continent.
  
  YEAR <- 2007
  
  gdp_by_continent <- function(d, yr) {
    sub <- d[d$year == yr, ]
    tapply(sub$gdpPercap, sub$continent, mean)
  }
  ...
  ⎿ File created successfully at: code/gdp_by_continent.R
⏺ Bash(Rscript code/gdp_by_continent.R)
  ⎿ Exit code 1
       Africa  Americas      Asia    Europe   Oceania 
     3089.033 11003.032 12473.027 25054.482 29810.188 
...
⏺ Edit(code/gdp_by_continent.R)
  ...
  +     tapply(sub$pop, sub$continent, sum)
  + }
  ⎿ The file code/gdp_by_continent.R has been updated successfully.
⏺ Bash(Rscript code/gdp_by_continent.R)
  ⎿ Africa  Americas      Asia    Europe   Oceania 
     2560.930 21602.746  5432.372 25244.048 32884.555
⏺ Reply:
  Output of the last run (exit code 0, check passes):
  ...
  1. **First run** — `gdp_by_continent` computed the plain (unweighted) mean of `gdpPercap`. Oceania came out at 29,810.19, i.e. (34,435.37 + 25,185.01)/2, and `stopifnot` failed.
  2. **Fix** — the hand figure 32,883 is the population-weighted mean: (34,435.37 × 20,434,176 + 25,185.01 × 4,115,771) / 24,549,947 ≈ 32,884.6. I changed the function to weight by `pop` (same `tapply` pattern as `code/lifeexp_by_continent.R`) and updated the header comment to say so.
  
  Two things to flag:
  - Your request said "mean gdpPercap", but the check only passes for the population-weighted mean, so that is what the final function computes. If you actually want the unweighted mean, the check constant should be 29,810 instead.
  - The project CLAUDE.md says not to run scripts, but you explicitly asked for `Rscript` here, so I ran it. No git was used.
````
...
## The reviewer subagent's report {#reviewer-report}

```{.markdown filename="Prompt to paste into Claude Code (captured run)"}
Use the hand-check-reviewer subagent to review code/lifeexp_by_continent.R
against this rule: each country's life expectancy must be weighted by its
population, the pop column. Report what the subagent says. Do not edit
anything.
```

```{.default filename="The subagent's report, as returned"}
Rule: each country's life expectancy must be weighted by its population, the `pop`
  column.
Line (`code/lifeexp_by_continent.R`, lines 5-6): `tapply(sub$lifeExp * sub$pop,
  sub$continent, sum) / tapply(sub$pop, sub$continent, sum)` — sum of lifeExp times
  pop, divided by sum of pop, per continent.
Verdict: follows.
```

- Rule, line, verdict. Nothing edited: `git status` prints clean.
- It read the file with fresh eyes; the quoted line is what you check.

::: {.content-visible when-format="revealjs"}

::: {.notes}
Two things to read: the quoted line, against the file; and git status,
which proves the fence held.
:::
...
The captured run. The main conversation started the subagent with the
rule and the file name; the subagent read the file and returned the
three lines above; the main conversation relayed them and added that
nothing was edited. `git status` in Terminal 2 confirms it. The report
quotes the line, so the check is quick: open the file and read lines 5
and 6. A report that names a verdict without quoting a line is a
transcript claim; ask for the line.

Practice task 10 runs the same reviewer on `lifeexp_gain.R` against the
gain rule, where the answer depends on two lines, the `merge()` and the
subtraction.
...
- **9–10** A loop prompt run to a pass; a reviewer subagent on `lifeexp_gain.R`
...
# Loops and subagents
#  9. Task 4 again, as a loop. If task 4 left code/gdp_by_continent.R,
#     delete it first. Paste the loop prompt from the page (it keeps
#     "the mean gdpPercap", ambiguous on purpose, and puts the hand check
#     in the script). Count the Bash boxes. Then git diff: what did it
#     change to make the check pass? Delete the file and repeat with
#     29810 in the check. What passes now?
# 10. Create .claude/agents/hand-check-reviewer.md from the page, commit
#     it, relaunch, and ask the subagent to review code/lifeexp_gain.R
#     against this rule: "a gain is lifeExp in year to minus lifeExp in
#     year from, matched by country name". Does the report quote the
#     subtraction line and the matching line? git status afterwards.
```
...
# 9
#   Two Rscript boxes and one edit box: the first run prints 29810.19
#   and stops at the check; the edit weights by pop; the second run
#   prints 32884.56. With 29810 in the check, the first run passes and
#   the function stays unweighted. The check is the specification.
# expected: two runs, one patch; with the wrong constant, wrong code passes

# 10
#   Three lines: the rule; the merge() line and the subtraction
#   (both$lifeExp_to - both$lifeExp_from) quoted; "follows". git status
#   prints clean: the subagent has no edit tool.
# expected: rule, line, verdict; nothing changed on disk
```
...
| A loop | the check in the script, from your paper; "run it with Rscript; if the check fails, change the function and run again; show me the output of the last run" |
...
| loop | the tool runs the check itself and patches until it passes; the check becomes the specification | [§](#loop-run) |
