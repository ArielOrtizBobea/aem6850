...
| `S06-desktop-app-cowork.png` | The desktop app's message box on the Cowork tab, nothing typed: "How can I help you today?", **+**, the **Chat / Cowork** toggle with Cowork selected, and under the box **Project or folder** and the approval setting **Auto** | Captured 2026-09-15 on the instructor's Max account, 1x display (1920x1080), cropped to the left of the composer and upscaled 2x; the sidebar (project and chat names) is cropped off. A Retina capture would be sharper. |
| `S07-cowork-folder-connected.png` | The same box after the folder is connected: the chip **gapminder-practice** under the box next to **Auto** | The click path that produced it: Project or folder > New project > Use a folder > pick the folder > the app asks "Allow Claude to change files in gapminder-practice?" (Cancel / Always allow / Allow) > Create project. Add folder is the shorter route. The approval setting was left on Auto in the capture; the page tells students to switch it to the one that asks. |
